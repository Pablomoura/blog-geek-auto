...
export default async function NoticiaPage(props: NoticiaPageProps) {
  ...
  try {
    ...
    for (const fileName of allFiles) {
      ...
      const tempoLeitura = Math.ceil(relatedContent.split(/\s+/).length / 200);
                      ...
                      >
                        {post.thumb && (
                          <img
                            src={post.thumb}
                            alt={post.titulo}
                            className="w-32 h-24 object-cover flex-shrink-0 rounded-md"
                          />
                        )}
                        <div className="flex flex-col justify-center">
                          <p className="text-orange-500 text-xs font-bold uppercase mb-1">{post.categoria}</p>
                          <h3 className="text-sm font-semibold text-neutral-900 dark:text-white leading-snug">{post.titulo}</h3>
