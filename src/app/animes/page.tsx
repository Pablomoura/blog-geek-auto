...
export const metadata: Metadata = {
  title: "Animes e Mangás - GeekNews",
  description: "As melhores notícias, guias e novidades sobre animes e mangás no GeekNews. Fique por dentro dos lançamentos e recomendações.",
  keywords: ["animes", "mangás", "guia de animes", "notícias de anime", "melhores animes 2025"],
  alternates: {
    canonical: "https://www.geeknews.com.br/animes",
  },
};
...
type BannerCache = Record<string, Banner[]>;

async function carregarCacheBanners(): Promise<Banner[]> {
  try {
    const raw = await fs.readFile(path.join(process.cwd(), "public/cache-banners.json"), "utf-8");
    const cache: BannerCache = JSON.parse(raw);
    return Array.isArray(cache["mangas-e-animes"]) ? cache["mangas-e-animes"] : [];
  } catch {
    return [];
  }
}

export default async function PaginaAnimes({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  const banners = await carregarCacheBanners();
  ...
  for (const nomeArquivo of arquivos) {
    ...
    if (
      ...
      data.title &&
      data.thumb &&
      data.categoria === "MANGÁS E ANIMES" &&
      data.midia &&
      data.tipoMidia &&
      data.data &&
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        <h1 className="text-3xl font-extrabold mb-4 text-orange-500">🧡 Animes e Mangás</h1>
        <p className="mb-8 text-gray-700 dark:text-gray-300">
          Descubra lançamentos, análises e guias completos sobre os melhores animes e mangás do momento.
        </p>

        {banners.length > 0 && (
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            {/* Banner grande (mais recente) */}
            <Link
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <aside className="w-full lg:w-[300px] flex-shrink-0 space-y-10">
            <ProdutosAmazon categoria="MANGÁS E ANIMES" />
