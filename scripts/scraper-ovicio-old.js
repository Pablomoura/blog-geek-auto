...
function extrairThumbsFromMedia(media) {
  if (!media) return { midia: "/images/default.jpg", thumb: "/images/default.jpg", tipoMidia: "imagem" };

  // Caso media:content seja array (vários)
  const primary = Array.isArray(media) ? media[0] : media;

  const url = primary?.url || "";
  const isYouTube = url.includes("youtube.com/embed");
  const thumb = primary?.["media:thumbnail"]?.url || url;

  return {
    midia: url || "/images/default.jpg",
    thumb: thumb || "/images/default.jpg",
    tipoMidia: isYouTube ? "video" : "imagem",
  };
}
...
function limparTexto(html) {
  ...
  document.querySelectorAll("img").forEach((img) => {
    ...
    if (src && !imagens.includes(src) && src.startsWith("http")) {
        imagens.push(src);
    }
    });
...
async function processarRSS() {
  ...
  for (const noticia of items) {
    ...
    const media = noticia["media:content"];
    const { midia, thumb, tipoMidia } = extrairThumbsFromMedia(media);
